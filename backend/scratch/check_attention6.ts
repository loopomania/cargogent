...
async function run() {
  ...
  for (const row of qsRes.rows.slice(0, 10)) {
    const logRes = await pool.query(`
       SELECT error_message, airline_status
       FROM query_logs
       WHERE awb = $1 AND (error_message IS NOT NULL OR airline_status = 'Error' OR airline_status = 'Timeout')
       ORDER BY created_at DESC
       LIMIT 1
    `, [row.mawb]);
    console.log(`MAWB: ${row.mawb}`);
    if (logRes.rows.length > 0) {
      console.log(`  Airline Status: ${logRes.rows[0].airline_status}`);
      console.log(`  Last Error Log: ${logRes.rows[0].error_message}`);
    } else {
