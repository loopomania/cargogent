...
async function run() {
  ...
  let markdown = `# Shipments with Multiple Paths\n\n`;
  markdown += `*This report lists all shipments whose timeline events result in multiple split paths (flows) in the UI graph. This usually occurs due to missing airline segments, conflicting ground handler origins, or duplicate flight numbers with different locations.*\n\n`;
  markdown += `| MAWB | HAWB | Flow Count | Path Legs |\n|---|---|---|---|\n`;
  ...
  for (const row of res.rows) {
      ...
      if (flows.length > 1) {
          multiPathCount++;
          const prefix = row.mawb ? row.mawb.substring(0, 3) : 'UNK';
          prefixCounts[prefix] = (prefixCounts[prefix] || 0) + 1;
          
          const pathStrs = legs.map(l => `${l.from}->${l.to}`).join(', ');
          markdown += `| ${row.mawb} | ${row.hawb || 'N/A'} | ${flows.length} | ${pathStrs} |\n`;
      }
  }
  
  markdown += `\n## Breakdown by Airline Prefix\n`;
  Object.entries(prefixCounts).sort((a, b) => b[1] - a[1]).forEach(([prefix, count]) => {
      markdown += `- **${prefix}**: ${count} shipments\n`;
  });
  
  fs.writeFileSync('/tmp/multi_path_report.md', markdown);
  console.log(`Wrote artifact with ${multiPathCount} shipments.`);
  await client.end();
}
