...
async function run() {
  ...
  const res = await client.query(`
    ...
    dlv_events AS (
        SELECT mawb, hawb
        FROM query_events
        WHERE status_code = 'DLV' OR status_code = 'AWD' OR status_code = 'NFD' OR status_code = 'RCF'
        GROUP BY mawb, hawb
    )
    SELECT 
        d.mawb, 
  ...
  console.log('--- Shipments not delivered (in DB or by airline) > 3 days after DEP ---');
  console.log(`Total count: ${res.rows.length}`);
  if (res.rows.length > 0) {
      console.log('Top 10 oldest:');
      res.rows.slice(0, 10).forEach(r => {
          console.log(`- MAWB: ${r.mawb} | HAWB: ${r.hawb || 'N/A'} | DEP Time: ${r.first_dep_time.toISOString().split('T')[0]} | Days since DEP: ${Math.round(r.days_since_dep)} | Status: ${r.aggregated_status}`);
      });
  }
