...
async function run() {
  const qsRes = await pool.query("SELECT hawb FROM query_schedule WHERE mawb = '69559552102'");
  ...
  if (lsRes.rows.length > 0) {
    ...
    const eventsRes = await pool.query(
      "SELECT payload FROM query_events WHERE mawb = '69559552102' ORDER BY created_at ASC"
    );
    console.log("EVENTS:");
