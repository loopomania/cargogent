...
for (const norm of uniqueNormFlights) {
      ...
      for (const e of evs) {
          ...
          if (idx > 0 && maxCycleSoFar >= 5 && idx <= 4) { // Using the OLD bugged logic
...
console.log("Raw Flows generated:", flows.length);
flows.forEach((f, i) => {
  console.log(`Flow ${i+1}: ${f.map(l => `${l.from}->${l.to} (${l.flightNo})`).join(' , ')}`);
});
