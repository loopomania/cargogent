...
function buildFlows(legs: any[], origin: string): any[][] {
  ...
  // PRUNE INVALID MULTIPATHS
  const seenFirstLegs = new Set<string>();
  const filteredFlows: any[][] = [];
  flows.sort((a, b) => b.length - a.length);

  for (const flow of flows) {
     const startLeg = flow[0];
     const legKey = `${startLeg.to}-${startLeg.flightNo || 'NOFLIGHT'}`;
...
async function run() {
  ...
  console.log(`\nTotal remaining 114 shipments with multiple paths: ${multiCount}`);
  await client.end();
}
