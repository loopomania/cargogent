...
def scrape_elal_stealth(awb_full: str):
    ...
    with sync_playwright() as p:
        # Crucial args to avoid normal Chrome headless detection
        ...
        print(f"[*] Fetching {url} with stealth headless flags...")
        try:
            page.goto(url, wait_until="domcontentloaded", timeout=45000)
            page.wait_for_timeout(10000) # Give extra time for JS chalenge
    ...
    if "kramericaindustries" in html.lower() or "rbzns" in html.lower():
        print("[-] Radware bot block STILL caught us. Pure headless might be permanently blocked by Radware on this site.")
    else:
