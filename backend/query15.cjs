...
async function run() {
  ...
  const res = await client.query(`UPDATE query_schedule SET is_halted = false, error_count_consecutive = 0, stale_alert_sent = false, last_changed_at = NOW(), next_status_check_at = NOW() WHERE mawb = '69559552286'`);
